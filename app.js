(() => {
  ...
  // Responsive layout: detect both viewport class and actual aspect ratio.
  // This is deliberately JS + CSS so browser zoom/orientation changes are handled too.
  const updateViewportProfile = () => {
    const w = Math.max(document.documentElement.clientWidth, window.innerWidth || 0);
    const h = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
    const ratio = h ? w / h : 1;
    const device = w <= 600 ? 'phone' : w <= 900 ? 'tablet' : 'desktop';
    const orientation = ratio >= 1 ? 'landscape' : 'portrait';
    document.body.dataset.device = device;
    document.body.dataset.orientation = orientation;
    document.body.dataset.ratio = ratio.toFixed(3);
    document.documentElement.style.setProperty('--viewport-w', `${w}px`);
    document.documentElement.style.setProperty('--viewport-h', `${h}px`);
    document.documentElement.style.setProperty('--viewport-ratio', ratio.toFixed(3));
    const label = document.querySelector('#device-profile');
    const ratioLabel = document.querySelector('#device-ratio');
    if (label) label.textContent = `${device[0].toUpperCase()+device.slice(1)} • ${orientation}`;
    if (ratioLabel) ratioLabel.textContent = `${w}×${h} • ${ratio.toFixed(2)}:1`;
  };
  updateViewportProfile();
  window.addEventListener('resize', updateViewportProfile, {passive:true});
  window.addEventListener('orientationchange', () => setTimeout(updateViewportProfile, 80), {passive:true});
  if (window.visualViewport) window.visualViewport.addEventListener('resize', updateViewportProfile, {passive:true});
  ...
  const setBusy = value => {
    ...
    document.querySelectorAll('.tab,.chip,#provider,.tool-button').forEach(x => x.disabled = value);
  };
  ...
  function isBusyError(error) {
    const msg = String(error?.message || error || '');
    return /(queue|queued|busy|capacity|gpu.*(busy|available|limit)|temporarily unavailable|try again later|429|503)/i.test(msg);
  }

  function busyMessage(error) {
    const msg = String(error?.message || error || '');
    if (/429/.test(msg)) return 'Server request များနေပါတယ် (429) — ခဏစောင့်ပြီး ပြန်ကြိုးစားနေပါတယ်…';
    return 'AI server/GPU busy ဖြစ်နေပါတယ် — queue ထဲမှာ ပြန်စောင့်ပြီး အလိုအလျောက် စမ်းနေပါတယ်…';
  }
  ...
  async function withRetry(fn, attempts=4) {
    ...
    for (let n=1; n<=attempts; n++) {
      try { return await fn(n); }
        ...
        if (n < attempts) {
          const delay = isBusyError(e)
            ? Math.min(15000, 1800 * (2 ** (n - 1)) + Math.random() * 500)
            : Math.min(7000, 900 * n);
          setStatus(isBusyError(e) ? busyMessage(e) : `ပြန်ကြိုးစားနေသည်… (${n+1}/${attempts})`);
  ...
  async function runLongText() {
    ...
    // Connect one-by-one. Parallel connection bursts can make a shared Space look
    // overloaded and are unnecessary because generation itself is strictly sequential.
    for (const eng of engines) {
      try {
        await getClient(PROVIDERS[eng]);
      } catch (e) {
        if (eng === engines.at(-1)) throw e;
      }
    }
  ...
  // ─────────────────────────────────────────────────────────────
  // VIDEO → MYANMAR TRANSCRIPT SIDEBAR
  // Uses a public Faster-Whisper WebUI Space as the remote worker.
  // The endpoint is discovered at runtime so small Gradio UI changes
  // do not require hard-coding one fragile /predict signature.
  // ─────────────────────────────────────────────────────────────
  const TRANSCRIPT_SPACE = 'avans06/whisper-webui-translate';
  let transcriptClient = null;
  let transcriptApiInfo = null;
  ...
  const normalizeNaturalMyanmar = value => {
    ...
    // Keep the model's wording intact; only clean punctuation/spacing so we do not
    // accidentally change names, numbers, or meaning.
  ...
  async function getTranscriptClient(){
    if (!gradio) gradio = await import('https://cdn.jsdelivr.net/npm/@gradio/client@2.5.1/dist/index.min.js');
    if (!transcriptClient) transcriptClient = await gradio.Client.connect(TRANSCRIPT_SPACE, (()=>{const t=getHFToken();return t?{hf_token:t}:undefined;})());
    if (!transcriptApiInfo) transcriptApiInfo = await transcriptClient.view_api();
  ...
  function allTranscriptEndpoints(info){
    ...
    for(const [idx,ep] of Object.entries(info?.unnamed_endpoints||{})) out.push({name:typeof idx==='string'&&idx.startsWith('/')?idx:`${idx}`,ep});
    return out;
  }
  function pickTranscriptEndpoint(info){
    const endpoints=allTranscriptEndpoints(info);
    const score=(x)=>{
      const labels=(x.ep?.parameters||[]).map(p=>String(p.label||'').toLowerCase()).join(' | ');
      let n=0;
      if(labels.includes('whisper - model')) n+=6;
      if(labels.includes('whisper - language')) n+=6;
      if(labels.includes('download from url')) n+=8;
      if(labels.includes('task')) n+=5;
      if(labels.includes('vad')) n+=3;
      if(labels.includes('transcription')) n-=8;
      return n;
    };
    endpoints.sort((a,b)=>score(b)-score(a));
    const best=endpoints[0];
    if(!best || score(best)<10) throw Error('Whisper transcript API endpoint ကို ရှာမတွေ့ပါ');
    return best;
  }
  const labelOf=p=>String(p?.label||'').toLowerCase();
  function exampleOr(p, fallback){
    if(p && p.example_input !== undefined && p.example_input !== null && p.example_input !== '') return p.example_input;
    return fallback;
  }
  function buildTranscriptPayload(ep, url){
    const params=ep?.parameters||[];
    return params.map((p,i)=>{
      const l=labelOf(p);
      if(l.includes('whisper - model')) return exampleOr(p,'turbo');
      if(l.includes('whisper - language')) return $('#transcript-lang')?.value || '';
      if(l.includes('download from url')) return url;
      if(l.includes('upload files')) return [];
      if(l.includes('microphone input')) return null;
      if(l === 'task' || l.includes('select the task')) return 'transcribe';
      if(l === 'vad' || l.includes('voice activity')) return $('#transcript-vad')?.checked ? 'silero-vad' : 'none';
      if(l.includes('vad - merge window')) return 5;
      if(l.includes('vad - max merge size')) return 90;
      if(l.includes('vad - process timeout')) return 1800;
      if(l.includes('vad - padding')) return 1;
      if(l.includes('vad - prompt window')) return 1;
      if(l.includes('vad - initial prompt mode')) return 'prepend_first_segment';
      if(l.includes('word timestamps')) return false;
      if(l.includes('highlight words')) return false;
      if(l.includes('diarization - speakers')) return 2;
      if(l.includes('diarization - min speakers')) return 1;
      if(l.includes('diarization - max speakers')) return 8;
      if(l === 'diarization' || l.includes('whether to perform speaker diarization')) return false;
      if(l.includes('translation - batch size')) return 8;
      if(l.includes('translation - no repeat ngram')) return 3;
      if(l.includes('translation - num beams')) return 4;
      if(l.includes('translation - torch dtype')) return false;
      if(l.includes('translation - using bitsandbytes')) return null;
      if(l.includes('m2m100 - model')) return exampleOr(p,'m2m100_418M/facebook');
      if(l.includes('m2m100 - language')) return 'Burmese';
      if(l.includes('nllb - model')) return exampleOr(p,'nllb-200-distilled-600M/facebook');
      if(l.includes('nllb - language')) return 'Burmese';
      if(l.includes('mt5 - model')) return exampleOr(p,'');
      if(l.includes('mt5 - language')) return '';
      if(l.includes('alma - model')) return exampleOr(p,'');
      if(l.includes('alma - language')) return '';
      if(l.includes('madlad400 - model')) return exampleOr(p,'');
      if(l.includes('madlad400 - language')) return 'Burmese';
      if(l.includes('seamless - model')) return exampleOr(p,'');
      if(l.includes('seamless - language')) return '';
      if(l.includes('llama - model')) return exampleOr(p,'');
      if(l.includes('llama - language')) return 'Burmese';
      // Gradio State values used by this Space.
      if(l.includes('translateinput')) return 'nllb';
      if(l.includes('sourceinput')) return 'urlData';
      if(l.includes('translation') && l.includes('bilingual')) return false;
      if(l.includes('translation') && l.includes('detect line breaks')) return true;
      // Safe defaults for optional advanced controls.
      const typ=String(p?.type||'').toLowerCase();
      if(typ.includes('boolean')) return false;
      if(typ.includes('number')||typ.includes('integer')) return exampleOr(p,0);
      if(typ.includes('file')) return [];
      return exampleOr(p,'');
    });
  }
  ...
  function extractTranscriptResult(result){
    ...
    if(!Array.isArray(data)) return {text:String(data||''),srt:''};
    const strings=data.filter(x=>typeof x==='string');
    const textCandidate=strings.find(x=>/[\u1000-\u109F]/.test(x)) || strings.find(x=>x.trim().length>0) || '';
    const srtCandidate=strings.find(x=>/\d{2}:\d{2}:\d{2}[,.]\d{3}\s+-->/.test(x)) || '';
    return {text:normalizeNaturalMyanmar(textCandidate),srt:srtCandidate};
  }
  ...
  function setupTranscriptSidebar(){
    ...
    $('#transcript-download')?.addEventListener('click',()=>{const v=$('#transcript-output').value;if(!v)return;const blob=new Blob([v],{type:'text/plain;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`myanmar-transcript-${Date.now()}.txt`;a.click();});
    $('#transcript-use')?.addEventListener('click',()=>{const v=$('#transcript-output').value.trim();if(!v)return;text.value=v;text.dispatchEvent(new Event('input'));setTranscriptStatus('Transcript ကို main Text-to-Speech စာသားထဲ ထည့်ပြီးပါပြီ','ok');setOpen(false);text.focus();});
    $('#transcript-generate')?.addEventListener('click', async()=>{
      if(transcriptBusy)return;
      const url=$('#video-url').value.trim();
      if(!/^https?:\/\//i.test(url)) return setTranscriptStatus('Video URL မှန်မှန်ထည့်ပါ','error');
      transcriptBusy=true; $('#transcript-generate').disabled=true; $('#transcript-label').hidden=true; $('#transcript-spinner').hidden=false; $('#transcript-copy').disabled=true; $('#transcript-download').disabled=true; $('#transcript-use').disabled=true; transcriptProgress(3,'Whisper service ချိတ်နေသည်…');
      try{
        const client=await getTranscriptClient();
        const chosen=pickTranscriptEndpoint(transcriptApiInfo);
        setTranscriptStatus('Video ကို download လုပ်ပြီး အသံခွဲနေသည်…'); transcriptProgress(12,'Video → audio');
        const payload=buildTranscriptPayload(chosen.ep,url);
        let result;
        try {
          result = await withRetry(() => client.predict(chosen.name,payload), 4);
        } catch (e) {
          if (isBusyError(e)) throw Error('Transcript server busy ဖြစ်နေပါတယ်။ ခဏနားပြီး ပြန်စမ်းပါ။');
          throw e;
        }
        transcriptProgress(92,'Transcript ပြန်ရယူနေသည်…');
        const out=extractTranscriptResult(result);
        ...
        transcriptProgress(100,'ပြီးပါပြီ'); setTranscriptStatus('မြန်မာ Transcript ပြီးပါပြီ — စကားပြောသလို စီထားပေးပါတယ်','ok');
  ...
  // General tool sidebar: one place to switch between every available workflow.
  function setupToolSidebar() {
    const side = $('#transcript-sidebar');
    const back = $('#sidebar-backdrop');
    const open = () => { side?.classList.add('open'); back?.classList.add('open'); side?.setAttribute('aria-hidden','false'); };
    const close = () => { side?.classList.remove('open'); back?.classList.remove('open'); side?.setAttribute('aria-hidden','true'); };
    $('#transcript-toggle')?.addEventListener('click', open);
    $('#transcript-close')?.addEventListener('click', close);
    back?.addEventListener('click', close);

    document.querySelectorAll('.tool-button').forEach(btn => btn.addEventListener('click', () => {
      if (busy) return;
      const tool = btn.dataset.tool;
      document.querySelectorAll('.tool-button').forEach(x => x.classList.toggle('active', x === btn));
      if (tool === 'tts' || tool === 'clone') {
        const tab = document.querySelector(`.tab[data-mode="${tool}"]`);
        tab?.click();
        $('#text-input')?.focus();
        close();
        ...
        $('#video-url')?.focus();
        document.querySelector('.transcript-pane')?.scrollIntoView({behavior:'smooth', block:'nearest'});
      } else if (tool === 'history') {
        close();
        $('#history-list')?.scrollIntoView({behavior:'smooth', block:'center'});
      } else if (tool === 'settings') {
        close();
        const body = $('#advanced-body');
        if (body) body.hidden = false;
        $('#toggle-advanced').textContent = 'ပိတ်မည်';
        $('#advanced-body')?.scrollIntoView({behavior:'smooth', block:'center'});
      }
    }));
  }
  setupToolSidebar();
