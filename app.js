(() => {
  ...
  function cleanExternalTranscript(raw){
    let s=String(raw||'').replace(/\r/g,'').replace(/```(?:markdown|text)?/gi,'').replace(/```/g,'').trim();
    const lines=s.split('\n').map(x=>x.trim()).filter(Boolean);
    const timed=lines.filter(x=>/^\[\d{1,4}:\d{2}(?::\d{2})?\]\s*/.test(x));
    if(timed.length){
      s=timed.map(x=>x.replace(/^\[[^\]]+\]\s*/,'')).join('\n');
    }else{
      s=lines.filter(x=>
        !/^#\s*Transcript:/i.test(x) &&
        !/^Source video:/i.test(x) &&
        !/^Language:/i.test(x) &&
        !/^Other available languages:/i.test(x) &&
        !/^To request a specific language:/i.test(x)
      ).join('\n');
    }
    return normalizeNaturalMyanmar(s);
  }

  async function fetchExternalYouTubeTranscript(url){
    const id=getYouTubeVideoId(url);
    if(!id) throw Error('YouTube video ID ကိုမဖတ်နိုင်ပါ');
    const lang=transcriptLangCode();
    let lastErr=null;
    for(const provider of EXTERNAL_YT_TRANSCRIPT){
      ...
      try{
        ...
        const res=await fetch(provider.makeUrl(id,lang),{
        ...
        try{
          ...
          const walk=v=>{
            ...
            if(typeof v==='object'){
              ...
              if(Array.isArray(v.transcript)) v.transcript.forEach(walk);
              if(Array.isArray(v.segments)) v.segments.forEach(walk);
        ...
        transcript=cleanExternalTranscript(transcript);
        if(!transcript) throw Error(`${provider.name} က Transcript အလွတ်ပြန်ပေးသည်`);
        return {text:transcript,provider:provider.name};
      }catch(e){
        lastErr=e;
        console.warn('External YouTube transcript provider failed:',provider.name,e);
      }finally{
        clearTimeout(timer);
      }
    }
      ...
      try{
        ...
        if(source==='url' && getYouTubeVideoId(url)){
          try{
            ...
            const out=await fetchExternalYouTubeTranscript(url);
            ...
            transcriptProgress(100,'ပြီးပါပြီ');
            setTranscriptStatus(`${out.provider} နဲ့ Transcript ပြီးပါပြီ`,'ok');
