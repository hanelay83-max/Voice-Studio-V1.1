(() => {
  ...
  const TRANSCRIPT_ENGINES = [
    ...
    {space:'hf-audio/whisper-large-v3', label:'HF Audio Whisper Large V3'},
    {space:'hf-audio/whisper-large-v3-turbo', label:'Whisper Large V3 Turbo'},
    {space:'avans06/whisper-webui-translate', label:'Faster-Whisper WebUI'}
  ];
  ...
  function pickTranscriptEndpoint(info, kind='file'){
    ...
    const score=(x)=>{
      const labels=(x.ep?.parameters||[]).map(labelOf).join(' | ');
      let n=0;
      if(/transcrib/.test(x.name.toLowerCase())) n+=10;
      if(labels.includes('task')) n+=6;
      if(labels.includes('audio file')||labels.includes('audio')) n+=6;
      if(labels.includes('upload')) n+=4;
      if(labels.includes('youtube')||labels.includes('url')||labels.includes('download')) n += kind==='url' ? 10 : -5;
      if(labels.includes('whisper - model')) n+=4;
      if(labels.includes('whisper - language')) n+=4;
      if(labels.includes('translation')) n-=3;
      return n;
    };
    endpoints.sort((a,b)=>score(b)-score(a));
    if(!endpoints[0] || score(endpoints[0])<5) throw Error('Whisper API endpoint မတွေ့ပါ');
    return endpoints[0];
  }
  ...
  function buildWhisperPayload(ep, source, value, client, space){
    return (ep?.parameters||[]).map(p=>{
      const l=labelOf(p), typ=String(p?.type||'').toLowerCase();
      if(l.includes('youtube url') || l.includes('video url') || l.includes('download from url') || (l.includes('url') && l.includes('input'))) return value;
      ...
      if(l.includes('max filesize')) return 1000;
      if(l.includes('input') && (typ.includes('file') || typ.includes('audio') || l.includes('audio file') || l.includes('upload'))){
        return source==='file' ? gradio.handle_file(value) : null;
      }
      if(l.includes('upload files')) return source==='file' ? [gradio.handle_file(value)] : [];
      if(l.includes('microphone')) return null;
      if(l.includes('language')) return $('#transcript-lang')?.value || exampleOr(p,'');
      if(l.includes('speaker') || l.includes('diarization')) return false;
      if(l.includes('timestamp') || l.includes('highlight')) return false;
      if(l.includes('merge window')) return 5;
      if(l.includes('max merge size')) return 90;
      if(l.includes('process timeout')) return 1800;
      if(l.includes('padding')) return 1;
      if(l.includes('prompt window')) return 1;
      if(typ.includes('boolean')) return false;
      if(typ.includes('number')||typ.includes('integer')) return exampleOr(p,0);
      if(typ.includes('file')) return null;
      return exampleOr(p,'');
    });
  }
  ...
  async function extractVideoAudio(file){
    ...
    // Browser-native video playback + capture converts MP4/MKV/WebM/etc. to WebM/Opus
    // without requiring a server-side ffmpeg endpoint.
    ...
    if(!video.captureStream) { URL.revokeObjectURL(url); throw Error('ဒီ browser က local video audio extraction ကို မထောက်ပံ့ပါ။ MP3/WAV/M4A audio ကို တင်ပါ'); }
    const stream=video.captureStream();
    const audioTracks=stream.getAudioTracks();
    if(!audioTracks.length){URL.revokeObjectURL(url); throw Error('Video ထဲမှာ audio track မတွေ့ပါ');}
    ...
    recorder.start(250);
    try { await video.play(); } catch(e) { recorder.stop(); URL.revokeObjectURL(url); throw Error('Video ကို play မလုပ်နိုင်ပါ'); }
  ...
  async function transcribeWithEngine(engine, source, value){
    const {client,info}=await getTranscriptClient(engine.space);
    const chosen=pickTranscriptEndpoint(info, source);
    const params=chosen.ep?.parameters||[];
    const labels=params.map(labelOf).join(' | ');
    // Prefer a dedicated YouTube/URL endpoint when present. For other public links,
    // fall back to the generic audio endpoint; if the Space cannot fetch the URL,
    // the next engine is tried.
    if(source==='url' && (labels.includes('youtube url')||labels.includes('download from url')||labels.includes('video url'))){
      const payload=buildWhisperPayload(chosen.ep,'url',value,client,engine.space);
      return extractTranscriptResult(await client.predict(chosen.name,payload));
    }
    if(source==='url') throw Error('ဒီ engine က URL input မထောက်ပံ့ပါ');
    const audioFile=await extractVideoAudio(value);
    const payload=buildWhisperPayload(chosen.ep,'file',audioFile,client,engine.space);
    return extractTranscriptResult(await client.predict(chosen.name,payload));
  }
