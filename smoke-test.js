...
ok('Transcript has output format selector', /id="transcript-format"/.test(html))
ok('Transcript has Simple Transcript option', /value="simple"/.test(html))
ok('Transcript has SRT Subtitle option', /value="srt"/.test(html))
ok('Transcript builds valid SRT output', /cuesToSrt/.test(app))
ok('Transcript downloads SRT with subtitle MIME type', /application\/x-subrip/.test(app))
ok('Transcript download extension follows format', /myanmar-transcript-\$\{Date\.now\(\)\}\.\$\{ext\}/.test(app))
ok('Removed unused transcript-natural option', !app.includes('transcript-natural'))
