...
The sidebar adds a Video Transcript workflow. It accepts a public video URL, sends it to the configured Faster-Whisper WebUI Space, and returns Burmese transcript text. The remote Space supports URL inputs and Whisper/VAD processing; its code also supports translation models including NLLB.

Note: video-host restrictions can prevent a remote Space from downloading a particular URL. The browser app itself does not bypass those restrictions.


## Fixes in this revision

- Responsive layout now detects phone/tablet/desktop plus portrait/landscape and updates on resize/orientation changes.
- Added a unified Tools sidebar: Text→Speech, Voice Cloning, Video Transcript, History, and Advanced Settings.
- Mobile sidebar becomes a bottom sheet so it does not squeeze the main UI.
- Shared Hugging Face/ZeroGPU calls are kept sequential and connection warm-up is no longer parallel.
- Busy/429/503/queue errors now use exponential backoff before failing.
- Static smoke tests cover HTML wiring, JavaScript syntax, responsive hooks, tool routing, retry handling, and history.
- The Hugging Face server itself cannot be made permanently available from browser code; if a public Space has no GPU capacity or quota, the UI now reports that state and retries safely. For reliable production usage, use your own/duplicated Space and authenticate with your HF token.
