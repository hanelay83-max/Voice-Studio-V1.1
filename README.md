...
The Transcript tool always converts the final transcript to Burmese (`my`), regardless of the video's original language. YouTube captions are fetched from the external transcript providers first; when captions are available, the app then translates the transcript into Burmese in chunks. Local/non-YouTube media still uses the Whisper fallback before Burmese translation. The output format can be selected as Simple Transcript (`.txt`) or SRT Subtitle (`.srt`). SRT keeps subtitle timecodes when the external source or Whisper result provides timing data.
